...
export function TrialBanner({ subscriptionStatus, trialEndsAt }: Props) {
  ...
  if (subscriptionStatus === "active") {
    return (
      <div className="bg-green-500/10 border border-green-500/20 rounded-xl p-4 flex items-center gap-3 flex-wrap">
        ...
        <button
          ...
          disabled={loading}
          className="px-4 py-2 border border-input rounded-lg text-sm font-medium hover:bg-secondary disabled:opacity-50"
        >
      ...
      <div className="bg-red-500/10 border border-red-500/20 rounded-xl p-4 flex items-center gap-3 flex-wrap">
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-xs text-red-600">
            A renovação da assinatura falhou — seus recursos estão suspensos até regularizar.
          </p>
        </div>
        <button
          onClick={handleManage}
